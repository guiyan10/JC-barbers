...
<main class="wrap">
    <section class="hero">
        <img src="{{ asset('images/logo.svg') }}" alt="Logo JC Barber" class="brand-logo">
        <div>
        ...
        <form method="POST" action="{{ route('agendamento.publico.store') }}">
            ...
            <div class="grid">
                ...
                <div class="field">
                    ...
                    <input id="telefone" name="telefone" type="text" required value="{{ old('telefone') }}" placeholder="(99) 99999-9999">
                ...
                <div class="field">
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" placeholder="voce@email.com">
                    ...
                    <select id="servico_id" name="servico_id" required>
                        ...
                        @foreach($servicos as $servico)
                            <option value="{{ $servico->id }}" @selected((string) old('servico_id') === (string) $servico->id)>
                                {{ $servico->nome }} - R$ {{ number_format((float) $servico->preco, 2, ',', '.') }}
                                ({{ $servico->duracao_estimada }} min)
                            </option>
                        @endforeach
                    </select>
                    @error('servico_id')<div class="error">{{ $message }}</div>@enderror
                </div>

                <div class="field full">
                    <label for="data_hora">Data e horario</label>
                    <input id="data_hora" name="data_hora" type="hidden" required value="{{ old('data_hora') }}">
                    ...
                    @error('data_hora')<div class="error">{{ $message }}</div>@enderror
                </div>

                <div class="field full">
                    <label for="observacoes">Observacoes (opcional)</label>
                    <textarea id="observacoes" name="observacoes" placeholder="Ex.: preferencia de barbeiro, detalhes do corte...">{{ old('observacoes') }}</textarea>
                    @error('observacoes')<div class="error">{{ $message }}</div>@enderror
                </div>
            </div>
...
<script>
    (function () {
        ...
        const horariosEndpoint = "{{ route('agendamento.publico.horarios') }}";

        const minDateTime = "{{ $minDateTime }}";
